...
const RelaySidebar = ({ setSelectedChatFriendId, users }) => {
  ...
  const logoutMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(
        error.reponse?.data?.message || "An error occured while logging out",
      );
    },
  });
  ...
  return (
    <aside className="flex flex-col gap-4 w-5/12 sm:w-4/12 lg:w-1/4 px-1 lg:px-2 py-4 shadow-r shadow-lg">
      {/* 1st part - Profile  */}
{/* //this is loading coming from user */}
      {isLoading ? (
    ...
    <div className="flex flex-row gap-2 w-full  py-1 ">
        {topics.map((topic) => (
          <span
            ...
            className={`${activeTopic === topic && "bg-gradient-to-r from-orange-500 via-pink-500 to-purple-500 duration-300 transition-all ease-in-out"} w-full lg:px-5 py-1 lg:py-2 rounded-xl cursor-pointer text-xs sm:text-sm lg:text-base text-white text-center`}
          >
            {topic}
          </span>
        ))}
      ...
      <ChatContacs
        ...
        setSelectedChatFriendId={setSelectedChatFriendId}
        users={users}
      />
